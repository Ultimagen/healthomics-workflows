...
- [Introduction](#introduction)
  ...
  - [List of FeatureMap fields](#list-of-featuremap-fields)
    - [Base calling quality (BCSQ)](#base-calling-quality-bcsq)
    - [Pre-filter annotations (FILT / FILT_BITMAP / FILTER)](#pre-filter-annotations-filt--filt_bitmap--filter)
...
First, all the SNVs in the input data (CRAM file) are exported to a vcf file denoted a FeatureMap (f1), where multiple annotation for every read supporting each SNV are collected. Additionally, a random sample of bases is collected and exported in the same format to a separate file (f2). For every randomly sampled base, an "X_ALT" base is drawn randomly from one of the three options (e.g. if an A base was drawn, X_ALT ∈ {C,G,T}), and the relevant features (e.g. BCSQ, X_HMER_REF) are calculated as if X_ALT was the reference base. Then, a series of filters is applied to each file, as detailed below, leaving in only high quality data. Finally, the randomly sample bases in f2 are filtered for bases matching the reference genome, which and annotated as True, and the SNVs in f1 are filtered for low VAF (≤5%) and high-coverage (≥20×) and annotated as False. 
...
#### Base calling quality (BCSQ)
Ultima sequencing’s flow-based sequencing by synthesis technology cycles through flows of non-terminated nucleotides in a repetitive cycles (T,G,C,A) and reads the length of each incorporated homopolymer to generate sequencing reads, which are measured in flow-space and converted to base space while error probabilities are reported in flow-space. Sequences differing by one substitution in base-space differ either by 2 “flows” (homopolymers measured on different bases), or more – some substitutions termed cycle skip motifs cause a shift in the number of sequencing cycles compared to the reference (see https://www.biorxiv.org/content/10.1101/2025.08.11.669689v1). To estimate the likelihood of a given SNV to be the result of a base calling error, the likelihood ratio of the reference allele and the alt allele is calculated in flow space, where the alt allele is equal to the reference allele with the SNV applied. BCSQ is calculated as:

$$
\mathrm{BCSQ} \equiv 10\left(\log_{10} P(\mathrm{read}\mid \mathrm{alt\ haplotype}) - \log_{10} P(\mathrm{read}\mid \mathrm{ref\ haplotype})\right)
$$

(note - the older X_SCORE field in older FeatureMap files was defined the same way, except for the prefactor of 10 that was absent)

Since the quality string values are generally binned to a few discrete values, the BCSQ distribution respectively looks concentrated in a few dominating values. Typically, most SNV have the maximal quality values, which translates to BCSQ~=80 (Q40 twice, in two flows) in the case of non-cycle-skip. In the case of cycle-skip SNVs the difference is extremely large and is capped at 100. The BCSQ feature typically has one of the highest feature importances (SHAP scores) as almost all real SNVs have perfect base calling scores while some of the artefactual SNV calls have low scores.

#### Pre-filter annotations (FILT / FILT_BITMAP / FILTER)
Every read in the output vcf is annotated with its pre-filter status. The aggregate value is FORMAT/FILT, where 1 is assigned to reads where all the pre-filters passed and 0 where at least one did not. FILT_BITMAP enumerates the specific pre-filters for every read. The filters themselves are given separately in the `{srsnv_metadata_json}` under the filters.positive key, where filters with type=quality constitute the pre-filter, and the order corresponds to the order in FILT_BITMAP. For example, consider the following:
```... 
"filtering_stats": {
  "positive": {
    "filters": [
...
The 3 quality filters are no_adj_ref_diff, bcsq_gt_40 and edist_le_5. A FILT_BITMAP value of 101, for example, indicates that a read passed no_adj_ref_diff and edist_le_5 but failed bcsq_gt_40, and will therefore be assigned FILT=0.

The vcf FILTER field will be assigned PreFiltered if FILT=0 for all the reads supporting the variant, and PASS otherwise.
