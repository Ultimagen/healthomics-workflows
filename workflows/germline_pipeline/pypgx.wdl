...
workflow PyPGx {
    input {
        ...
        File? model_onnx
        # EfficientDV read-orientation params. Defaults reproduce the current
        # single-read (Ultima flow) behavior; the "Call variants using
        # EfficientDV PE" use-case overrides them for paired-end reads.
        # Optional-typed (not defaulted) so AWS HealthOmics treats them as
        # optional parameters; the current single-read (SE) values are applied
        # inside the EfficientDV call via select_first. The "Call variants using
        # EfficientDV PE" use-case overrides them for paired-end reads.
        Boolean? run_haplotype_sampling
        File? ref_hapl
    ...
    parameter_meta {
        ...
        run_haplotype_sampling: {
            help: "EfficientDV: run haplotype sampling to create pangenome haplotypes (enabled by the PE use-case)",
            type: "Boolean",
            category: "input_advanced"
        }
        ref_hapl: {
            help: "EfficientDV: pangenome haplotypes (.hapl) file, required when run_haplotype_sampling is true",
            type: "File",
            category: "input_advanced"
        }
        ref_gbz_for_haplotypes: {
            help: "EfficientDV: pangenome graph (.gbz) file, required when run_haplotype_sampling is true",
            type: "File",
            category: "input_advanced"
        }
        num_haplotypes: {
            help: "EfficientDV: number of haplotypes to sample",
            type: "Int",
            category: "input_advanced"
        }
        include_reference_in_haplotypes: {
            help: "EfficientDV: include the reference among sampled haplotypes",
            type: "Boolean",
            category: "input_advanced"
        }
        diploid_sampling_in_haplotypes: {
            help: "EfficientDV: use diploid sampling for haplotypes",
            type: "Boolean",
            category: "input_advanced"
        }
        ensemble_size: {
            help: "EfficientDV: number of augmented passes for ensemble inference (0 disables)",
            type: "Int",
            category: "input_advanced"
        }
        single_strand_filter: {
            help: "EfficientDV: enable single-strand filtering (default set by template)",
            type: "Boolean",
            category: "input_advanced"
        }
        min_base_quality: {
            help: "EfficientDV: minimum base quality",
            type: "Int",
            category: "input_advanced"
        }
        min_fraction_single_strand_non_snps: {
            help: "EfficientDV: minimum fraction of single-strand support for non-SNP variants",
            type: "Float",
            category: "input_advanced"
        }
        ug_make_examples_extra_args: {
            help: "EfficientDV: extra args passed to make_examples (channels / max-ins-size for PE)",
            type: "String",
            category: "input_advanced"
        }
    ...
    if (!defined(input_vcf_file)) {
        ...
        input:
            ...
            num_shards = 20,
            single_strand_filter = select_first([single_strand_filter, false]),
            min_fraction_single_strand_non_snps = select_first([min_fraction_single_strand_non_snps, 0]),
            min_base_quality = select_first([min_base_quality, 5]),
            ensemble_size = select_first([ensemble_size, 0]),
            run_haplotype_sampling = select_first([run_haplotype_sampling, false]),
            ref_hapl = ref_hapl,
            ref_gbz_for_haplotypes = ref_gbz_for_haplotypes,
            num_haplotypes = num_haplotypes,
            include_reference_in_haplotypes = include_reference_in_haplotypes,
            diploid_sampling_in_haplotypes = diploid_sampling_in_haplotypes,
