...
workflow SVPipeline {
    input {
        ...
        # Winval validations
        #@wv reference_genome in {"hg38", "b37", "hg38_no_alt", "hg38_nist_v3_with_decoy", "mm39"}
        #@wv min_base >= 0
        #@wv min_mapq >= 0
        #@wv max_num_haps >= 0
        #@wv num_shards > 0
    ...
    parameter_meta {
        ...
        reference_genome: {
            type: "String",
            help: "Genome type selector. Supported values: hg38, b37, hg38_no_alt, hg38_nist_v3_with_decoy, mm39",
            category: "required"
        }
        ua_parameters: {
            type: "UaParameters",
    ...
    # Resolve genome-dependent SV resources from genome_resources, allowing per-run overrides
    File wgs_calling_interval_list = select_first([wgs_calling_interval_list_override, GenomeResources.resources[reference_genome].calling_interval_list_without_artefacts])
    File? blacklist_bed_resolved = if defined(blacklist_bed) then blacklist_bed else GenomeResources.resources[reference_genome].sv_blacklist
    ...
    call IdentifyVariants {
        input:
            ...
            blacklist_bed = blacklist_bed_resolved,
            gridss_docker = global.gridss_docker,
            preemptible_tries = preemptibles,
            monitoring_script = monitoring_script,
            no_address = no_address
...
task CreateAssembly {
    ...
    command <<<
        ...
        then
            ...
            ~{if is_somatic then "--somatic-active-areas" else ""} \
            ...
            --haps-max-sv-events 1 \
            --haps-hp-dedup true \
            --haps-read-witness true \
            --prog \
            --sv \
            --realigned-sam
