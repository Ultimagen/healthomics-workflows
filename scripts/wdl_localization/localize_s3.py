...
def copy_s3_file(s3_client, src_bucket, src_key, dest_bucket):
    dest_key = src_key  # keeping the same key in the destination bucket
...
def update_s3_paths(data, s3_client, dest_bucket):
    if isinstance(data, dict):
        for key, value in data.items():
            data[key] = update_s3_paths(value, s3_client, dest_bucket)
...
def update_wdl_s3_paths(wdl_content, s3_client, dest_bucket):
    s3_paths = parse_wdl_for_s3_paths(wdl_content)
    for path in s3_paths:
        src_bucket, src_key = re.match(r's3://([^/]+)/(.+)', path).groups()
        new_path = copy_s3_file(s3_client, src_bucket, src_key, dest_bucket)
        wdl_content = wdl_content.replace(path, new_path)
    return wdl_content


def localize_s3_files(input_file, dest_bucket):
    s3_client = boto3.client('s3')

    if input_file.endswith('.json'):
        with open(input_file, 'r') as file:
            data = json.load(file)
        updated_data = update_s3_paths(data, s3_client, dest_bucket)
        ...
        updated_wdl_content = update_wdl_s3_paths(wdl_content, s3_client, dest_bucket)
        with open(input_file, 'w') as file:
...
if __name__ == "__main__":
    ...
    localize_s3_files(args.input_file, args.dest_bucket)
